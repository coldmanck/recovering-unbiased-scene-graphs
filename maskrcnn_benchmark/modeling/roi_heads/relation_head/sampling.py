...
class RelationSampling(object):
    ...
    def prepare_test_pairs(self, device, proposals):
        ...
        for p in proposals:
            ...
            if (not self.use_gt_box) and self.test_overlap: # self.test_overlap is False
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        rel_idx_pairs = []
        rel_labels = []
        stl_rel_labels = [] if self.stl_train else None
        rel_sym_binarys = []
        # bg_start_idxs = [] if self.bg_soft_loss else None
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            if not self.multi_label_training:
                ...
                if self.stl_train:
                    ...
                    stl_rel_matrix = target.get_field("relation_stl") # [tgt, tgt]
                    stl_rel_label = stl_rel_matrix[tgt_bg_idxs[:, 0], tgt_bg_idxs[:, 1]]
                    stl_rel_labels.append(stl_rel_label)
                    # img_rel_labels = tgt_rel_labs.long().contiguous().view(-1)
                # elif self.bg_soft_loss:
                #     bg_start_idx = len(tgt_rel_labs)
                #     bg_start_idxs.append(bg_start_idx)
                ...
                relation_map_multi_label = target.get_field('relation_map_multi_label')
                rel_idx_pairs.append(tgt_pair_idxs)
                rel_labels.append(relation_map_multi_label[tgt_head_idxs, tgt_tail_idxs])
                # if any(relation_map_multi_label[tgt_head_idxs, tgt_tail_idxs].sum(1) != 1):
                #     import pdb; pdb.set_trace()

        # import pdb; pdb.set_trace()
        return proposals, rel_labels, rel_idx_pairs, rel_sym_binarys, stl_rel_labels # , bg_start_idxs
    ...
    def detect_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            if self.require_overlap and (not self.use_gt_box): # self.require_overlap is False by default
            ...
            # relation_stl_truth_table = target.get_field("relation_stl_truth_table") if self.stl_train else None
            relation_stl_truth_table = None
            img_rel_triplets, binary_rel, nb_fg_rels = self.motif_rel_fg_bg_sampling(device, tgt_rel_matrix, ious, is_match, rel_possibility, relation_stl_truth_table, relation_map_multi_label)
            rel_idx_pairs.append(img_rel_triplets[:, :2]) # (num_rel, 2),  (sub_idx, obj_idx)
            if self.stl_train:
                stl_rel_matrix = target.get_field("relation_stl") # [tgt, tgt]
                stl_rel_label = stl_rel_matrix[img_rel_triplets[nb_fg_rels:, 0], img_rel_triplets[nb_fg_rels:, 1]]
                stl_rel_labels.append(stl_rel_label)

                # rel_labels.append(img_rel_triplets[:nb_fg_rels, 2]) # (num_rel, )
            # else:
            if self.multi_label_training:
        ...
        return proposals, rel_labels, rel_idx_pairs, rel_sym_binarys, stl_rel_labels
    

    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility, relation_stl_truth_table, relation_map_multi_label):
        """
        ...
        for i in range(num_tgt_rels):
            ...
            if self.multi_label_training:
                fg_labels = torch.cat([relation_map_multi_label[tgt_head_idx, tgt_tail_idx]] * prp_tail_idxs.shape[0]).view(-1, 51)
        ...
        if self.multi_label_training:
            if len(fg_rel_triplets) == 0:
                fg_rel_triplets = torch.zeros((1, 2 + 51), dtype=torch.int64, device=device)
            else:
                fg_rel_triplets = cat(fg_rel_triplets, dim=0).to(torch.int64)
            return fg_rel_triplets, binary_rel, len(fg_rel_triplets)
        else:
            # select fg relations
            if len(fg_rel_triplets) == 0:
            ...
            # select bg relations
            if relation_stl_truth_table is not None:
                import pdb; pdb.set_trace()
            ...
            return cat((fg_rel_triplets, bg_rel_triplets), dim=0), binary_rel, len(fg_rel_triplets)
