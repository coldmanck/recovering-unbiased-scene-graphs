...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.cfg.MODEL.RELATION_ON:
            ...
            # During training, self.box() will return the unaltered proposals as "detections"
            # this makes the API consistent during training and testing
            x, detections, loss_relation, rel_features, rel_targets = self.relation(features, detections, targets, logger)
            losses.update(loss_relation)

        return x, detections, losses, rel_features, rel_targets
