...
_C.MODEL.STL_EVAL = False # whether to compute STL instead of general evaluation
_C.MODEL.STL_EVAL_GENERAL = False # whether to set bg labeling prob to one. Should has always been `True` actually.
_C.MODEL.STL_TRAIN = False # whether to conduct STL training
_C.MODEL.STL_TRAIN_NORM = False # whether to normalize soft label (exclude background -> normalized prob. of class 1-50 with the sum)
_C.MODEL.STL_TRAIN_LABEL_PATH = '' # path to predicted soft label. E.g. `output/motif-precls-exmp/inference/VG_stanford_filtered_with_attribute_test/train_eval_results.pytorch`.
_C.MODEL.STL_TRAIN_LOSS_ALPHA = 1.0 # the factor for stl loss

_C.MODEL.STL_EVAL_AVG_VAL_SET = False # whether averaging labeling probability of training set with that of validation set. Default False
_C.MODEL.STL_EVAL_ZERO_OUT_NAN = False # whether to zero out nan values and make final factored labeling prob 0
_C.MODEL.VIS_MODE = False # Can be True ONLY WHEN using jupyter notebook to visualize results

_C.MODEL.BG_SOFT_LOSS = False # whether to use soft cross entropy loss for background triplets
...
_C.MODEL.BALANCED_NORM_TRAIN_GX = False # Also train against g(x) in addition to f(x), against the training set p(s|x)
_C.MODEL.BALANCED_NORM_TRAIN_GX_LOSS_ALPHA = 1.0
_C.MODEL.BALANCED_NORM_LEARNABLE = False # Learn labeling probability
_C.MODEL.BALANCED_NORM_WITH_GRADIENT = False
_C.MODEL.BALANCED_NORM_AS_SOFT_LABEL = False
_C.MODEL.BALANCED_NORM_AS_SOFT_LABEL_SLL = False
_C.MODEL.BALANCED_NORM_AS_SOFT_LABEL_MLL = False
_C.MODEL.BALANCED_NORM_AS_SOFT_LABEL_NORMALIZE_LB = False
_C.MODEL.BALANCED_NORM_USE_BCELOSS = False
_C.MODEL.BALANCED_NORM_USE_MSELOSS = False

_C.MODEL.TRAIN_AVG_BELIEF_TO_ONE = False
_C.MODEL.TRAIN_AVG_BELIEF_TO_ONE_FACTOR = 1.0
_C.MODEL.TRAIN_AVG_BELIEF_TO_ONE_ONLY_THIS_LOSS = False

_C.MODEL.BALANCED_NORM_TEST_ONLY = False

# Use pre-computed (fixed) labeling probability instead of the dynamically estimated one
_C.MODEL.BALANCED_NORM_FIXED = False
_C.MODEL.BALANCED_NORM_FIXED_NORMALIZED = False
_C.MODEL.BALANCED_NORM_FIXED_MSELOSS = False
_C.MODEL.PRECOMPUTED_BALANCED_NORM_PATH = '' # e.g. 'output/motif-precls-exmp/inference/VG_stanford_filtered_with_attribute_test/train_labeling_prob.pt'

_C.MODEL.PCPL_CENTER_LOSS = False # Whether to apply PCPL with Center Loss as described in Yan et al. "PCPL: Predicate-Correlation Perception Learning for Unbiased Scene Graph Generation"
_C.MODEL.CENTER_LOSS_LR = 0.01
_C.MODEL.CENTER_LOSS_ALPHA = 0.5
_C.MODEL.CENTER_LOSS_LAMBDA = 0.03
...
_C.MODEL.ROI_RELATION_HEAD.CAUSAL.CONTEXT_LAYER = 'motifs' # 'motifs', 'vctree', 'vtranse'
...
# ---------------------------------------------------------------------------- #
_C.TRAIN = CN()
_C.TRAIN.CONCEPT_AUGMENTED = False # whether to use "Concept Augmented Training": add additional relationship examples with higher (concept) level predicates. E.g. <person-standing on-street> will produce <person-on-street>.
_C.TRAIN.COGTREE_PATH = '' # used for CAT training

# _C.TRAIN.REWEIGHT = False
# _C.TRAIN.RESAMPLE = False
# _C.TRAIN.FOCAL_LOSS = False
_C.TRAIN.UNBIASED_TRAINING = '' # 'reweight', 'resample' or 'focal_loss'
_C.TRAIN.PREDICATE_WEIGHTS_PATH = 'datasets/vg/predicate_weights.pkl' # generated from VG150 training split. Bg:fg ratio is assumed to be 3:1 so the weight of BG is 1/3.
_C.TRAIN.WEIGHT_FACTOR = 1

_C.TRAIN.BAL_REWEIGHT_ONLY = False # # Whether to only return estimated labeling probability without performing BalNorm

_C.TRAIN.BALNORM_REWEIGHT_LABEL_PROB = False # Can be True ONLY WHEN Balnorm & UNBIASED_TRAINING is enabled!
_C.TRAIN.BALNORM_REWEIGHT_INV_LABEL_PROB = False # Can be True ONLY WHEN Balnorm & UNBIASED_TRAINING is enabled!

_C.TRAIN.BALNORM_REWEIGHT_LABEL_DISTRIB = False # Can be True ONLY WHEN Balnorm & UNBIASED_TRAINING is enabled!
_C.TRAIN.NORM_PREDICATE_COUNTS_PATH = 'datasets/vg/normalized_predicate_counts.pkl' # generated from VG150 training split.
...
# Below are added for STL
_C.TEST.ALLOW_LOAD_LABEL_PROB_FROM_CACHE = True
_C.TEST.EVAL_BBOX = True

_C.TEST.CUSTUM_EVAL = False
_C.TEST.CUSTUM_PATH = '.'

# Below are for STL
_C.TEST.STL_MODE = False
_C.TEST.STL_MODE_GENERAL = True # False
_C.TEST.STL_MODE_PREDCLS_LP = False
_C.TEST.STL_TRAINING_SET_LABELING_PROB = ''

# Below are for CogTree
_C.TEST.COMP_COGTREE = False
_C.TEST.WITH_COGTREE = False
...
# ---------------------------------------------------------------------------- #
_C.HMC = CN()
_C.HMC.C_HMC_TRAIN = False # whether to use C-HMC loss (MCLoss) during training
_C.HMC.C_HMC_TEST = False # whether to inference in C-HMC manner, i.e., p_i = max(p_j*y_j) for j in C(i)
_C.HMC.ANCESTOR_MAT_PATH = '' # path to the matrix of ancestors (R) (generated by ipynb file)

_C.TRAIN.MULTI_LABEL_TRAINING = False # whether to do multi-label per object-pair training instead of per-triplet training
_C.TRAIN.MULTI_LABEL_NORM_LOSS = False # whether to use normalized one-versus-all loss
...
_C.LOG_TB = True # False # whether to use tensorboard to log losses
