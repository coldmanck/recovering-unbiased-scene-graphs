...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        self.mode = mode
        # soft transfer learning (STL)
        self.stl_train = stl_train
        if self.stl_train and self.split == 'train':
            self.stl_labels = torch.load(stl_train_label_path)['predictions'] # a list of BoxLists (57723 imgs)
            for boxlist in self.stl_labels:
                pred_soft_labels = boxlist.get_field('pred_soft_labels')
                if stl_eval_zero_out_nan: # default False
                    raw_labeling_prob_fname = 'train_labeling_prob_raw.pt'
                    raw_labeling_prob_path = '/'.join(stl_train_label_path.split('/')[:-1]) + '/' + raw_labeling_prob_fname
                    assert os.path.exists(raw_labeling_prob_path)
                    raw_labeling_prob = torch.load(raw_labeling_prob_path)
                    nan_idxs = torch.isnan(raw_labeling_prob)
                    pred_soft_labels[:, nan_idxs] = 0.0

                pred_soft_labels /= torch.sum(pred_soft_labels, dim=-1).view(-1, 1)
                boxlist.add_field('pred_soft_labels', pred_soft_labels)

        self.custom_eval = custom_eval
        self.concept_augmented_training = concept_augmented_training
        
        self.cogtree = None
        if self.concept_augmented_training:
            assert cogtree_path != ''
            with open(cogtree_path, 'rb') as f:
        ...
        #     self.hierarchy_mat = np.zeros(51, 51)
        #     np.fill_diagonal(self.hierarchy_mat, 1)
        ...
        if self.custom_eval:
            ...
            self.split_mask, self.gt_boxes, self.gt_classes, self.gt_attributes, self.relationships = load_graphs(
                ...
                concept_augmented_training=self.concept_augmented_training,
                cogtree=self.cogtree,
            )
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        if evaluation:
            ...
            if self.stl_train and self.split == 'train':
                # self.stl_labels[index].fields()
                # ['labels', 'attributes', 'pred_labels', 'pred_scores', 'rel_pair_idxs', 'pred_rel_scores', 'pred_rel_labels', 'pred_soft_labels', 'pred_rel_pair_idxs']
                if self.mode != 'sgdet':
                    pred = self.stl_labels[index]
                    stl_soft_labels = pred.get_field('pred_soft_labels')
                    stl_rel_idxs = pred.get_field('pred_rel_pair_idxs')
                    relation_map_stl = torch.zeros((num_box, num_box, 51), dtype=torch.float)
                    relation_map_stl[stl_rel_idxs[:, 0], stl_rel_idxs[:, 1]] = stl_soft_labels
                    target.add_field('relation_stl', relation_map_stl, is_triplet=True)
                
                # relation_stl_truth_table = torch.zeros((num_box, num_box), dtype=torch.int)
                # relation_stl_truth_table[stl_rel_idxs[:, 0], stl_rel_idxs[:, 1]] = 1
            ...
            if self.stl_train and self.split == 'train':
                if self.mode == 'sgdet':
                    ...
                    stl_soft_labels = pred.get_field('pred_soft_labels')
                    stl_rel_idxs = pred.get_field('pred_rel_pair_idxs')
                    relation_map_stl = torch.zeros((80, 80, 51), dtype=torch.float) # 80 = cfg.MODEL.ROI_HEADS.DETECTIONS_PER_IMG
                    relation_map_stl[stl_rel_idxs[:, 0], stl_rel_idxs[:, 1]] = stl_soft_labels
                    target.add_field('relation_stl', relation_map_stl, is_triplet=True)
            
                # For visualization purpose only
                if self.vis_mode:
                    target.add_field('stl_soft_labels', stl_soft_labels, is_triplet=True)
                    target.add_field('stl_rel_idxs', stl_rel_idxs, is_triplet=True)
                    pred_rel_scores = pred.get_field('pred_rel_scores')
                    rel_pair_idxs = pred.get_field('rel_pair_idxs')
                    target.add_field('pred_rel_scores', pred_rel_scores, is_triplet=True)
                    target.add_field('rel_pair_idxs', rel_pair_idxs, is_triplet=True)
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap, concept_augmented_training=False, cogtree=None):
    """
    ...
    for i in range(len(image_index)):
        ...
        if i_rel_start >= 0:
            ...
            if concept_augmented_training:
                rels_hmap = {tuple(rel) for rel in rels.tolist()}
                for rel in rels:
                    for concept in cogtree[rel[2]]:
                        if concept != rel[2] and (rel[0], rel[1], concept) not in rels_hmap:
                            rels = np.row_stack((rels, [[rel[0], rel[1], concept]]))
        else:
