...
def do_vg_evaluation(
    ...
    comp_labeling_prob = True if labeling_prob_path != '' else False
    
    # Settings for CogTree
    comp_cogtree = cfg.TEST.COMP_COGTREE
    test_with_cogtree = cfg.TEST.WITH_COGTREE
    cogtree_file = None
    ...
    groundtruths = []
    # if mode == 'sgdet' and output_folder is not None:
    #     gt_pred_idxs_path = os.path.join(output_folder, 'gt_pred_idxs.json')
    if comp_soft_label: # and mode != 'sgdet':
        new_predictions = []
        # if mode == 'sgdet':
        #     with open(gt_pred_idxs_path, 'r') as f:
        #         gt_pred_idxs = json.load(f)
    ...
    for image_id, prediction in enumerate(tqdm(predictions)):
        ...
        if comp_soft_label: # and 
            pred_rel_scores = prediction.get_field('pred_rel_scores')
            labeling_prob[0] = 1
            pred_soft_labels = labeling_prob.float() * pred_rel_scores / pred_rel_scores[:, 0].view(-1, 1)

            pred_rel_pair_idxs = prediction.get_field('rel_pair_idxs')

            new_prediction = BoxList(predictions[image_id].bbox, predictions[image_id].size, predictions[image_id].mode) # xyxy
            if mode != 'sgdet':    
                new_prediction.add_field('pred_soft_labels', pred_soft_labels, is_triplet=True)
                new_prediction.add_field('pred_rel_pair_idxs', pred_rel_pair_idxs, is_triplet=True)
            else:
                new_prediction.add_field('pred_soft_labels', pred_soft_labels, is_triplet=True)
                new_prediction.add_field('pred_rel_pair_idxs', pred_rel_pair_idxs, is_triplet=True)
            ...
            if cfg.DEBUG and image_id == 4:
                logger.info('Debug mode is ON! Break with only 5 images')
                break
            
    if comp_soft_label: # and mode != 'sgdet':
        assert output_folder
        torch.save({'predictions':new_predictions}, os.path.join(output_folder, 'soft-label_' + cache_name))
        logger.info('comp_soft_label is True! Break')
        return
    if not test_with_cogtree and not comp_cogtree:
        save_output(output_folder, groundtruths, predictions, dataset, cache_name)
    
    result_str = '\n' + '=' * 100 + '\n'
    if "bbox" in iou_types and cfg.TEST.EVAL_BBOX:
    ...
    if "relations" in iou_types:
        ...
        assert not (comp_cogtree and test_with_cogtree) # sanity check
        eval_recall = SGRecall(result_dict, comp_labeling_prob, comp_label_prob_for_sgdet=(comp_labeling_prob and mode == 'sgdet'), comp_cogtree=comp_cogtree, test_with_cogtree=test_with_cogtree, cogtree_file=cogtree_file)
        ...
        if comp_labeling_prob: # first STL phase
            stl_count = torch.zeros((51, ), dtype=torch.int64)
            stl_probs = torch.zeros((51, ), dtype=torch.float64)
            logger.info('Computing labeling probabilities...')

        if mode == 'sgdet':
            all_gt_pred_idxs = []
        if comp_cogtree:
            gt_rel_to_pred_rel = [[0 for _ in range(51)] for _ in range(51)]
        ...
        for groundtruth, prediction in tqdm(zip(groundtruths, predictions), total=len(groundtruths)):
            ...
            if comp_labeling_prob:
                # assert pred_scores is not None and pred_labels is not None
                # import pdb; pdb.set_trace()
                if pred_scores is not None and pred_labels is not None:
                    for pred_label, pred_score in zip(pred_labels, pred_scores):
                        # import pdb; pdb.set_trace()
                        stl_count[pred_label] += 1
                        stl_probs[pred_label] += pred_score
                        example_count[pred_label.item()] += 1
                # if mode == 'sgdet':
                #     assert gt_pred_idxs is not None
                #     all_gt_pred_idxs.append(gt_pred_idxs)
        if comp_labeling_prob and output_folder:
            example_count_path = os.path.join(output_folder, 'example_count.pkl')
            with open(example_count_path, 'wb') as f:
                pickle.dump(example_count, f)
        if comp_cogtree:
            with open(gt_rel_to_pred_rel_path, 'wb') as f:
                pickle.dump(gt_rel_to_pred_rel, f)
            logger.info(f'gt_rel_to_pred_rel saved to {gt_rel_to_pred_rel_path}')
        if comp_labeling_prob:
            # if mode == 'sgdet':
            #     with open(gt_pred_idxs_path, 'w') as f:
            #         json.dump(all_gt_pred_idxs, f)
            #     logger.info(f'gt_pred_idxs_path saved to {gt_pred_idxs_path}')

            raw_labeling_prob = stl_probs / stl_count
            raw_labeling_prob_path = os.path.join(output_folder, 'train_labeling_prob_raw.pt')
            torch.save(raw_labeling_prob, raw_labeling_prob_path)
            logger.info(f'Raw labeling probabilities saved to {raw_labeling_prob_path}')
            
            # compute median labeling prob to fill in nan values
            non_nan_probs = raw_labeling_prob[~torch.isnan(raw_labeling_prob)]
            median_prob = sorted(non_nan_probs)[len(non_nan_probs) // 2]
            raw_labeling_prob[torch.isnan(raw_labeling_prob)] = median_prob
            torch.save(raw_labeling_prob, labeling_prob_path)
            logger.info(f'Final labeling probabilities saved to {labeling_prob_path}')
    ...
    if "relations" in iou_types:
        if output_folder:
            if cfg.TEST.STL_MODE and test_with_cogtree:
                save_name = 'result_dict-stl-cogtree.pytorch'
            elif cfg.TEST.STL_MODE and cfg.TEST.STL_MODE_PREDCLS_LP:
                save_name = 'result_dict-stl-predcls_lp.pytorch'
            elif cfg.TEST.STL_MODE:
                save_name = 'result_dict-stl.pytorch'
            elif test_with_cogtree:
                save_name = 'result_dict-cogtree.pytorch'
            else:    
                save_name = 'result_dict.pytorch'
            torch.save(result_dict, os.path.join(output_folder, save_name))
        if monitor_mean_recall:
...
def save_output(output_folder, groundtruths, predictions, dataset, cache_name):
    if output_folder:
        torch.save({'groundtruths':groundtruths, 'predictions':predictions}, os.path.join(output_folder, cache_name))
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    return pred_scores, pred_labels, gt_pred_idxs, pred_predicates
