...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0, is_train=None, disable_transform=False):
    assert mode in {'train', 'val', 'test'}
    num_gpus = get_world_size()
    is_train = is_train if is_train is not None else (mode == 'train') # if is_train is given then overwrite current setting
    if is_train:
    ...
    transforms = None if (not is_train and cfg.TEST.BBOX_AUG.ENABLED) or disable_transform else build_transforms(cfg, is_train)
