...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None, debug=False):
    model.eval()
    results_dict = {}
    ...
    for i, batch in enumerate(tqdm(data_loader)):
        ...
        if debug and i == 4:
            print('Debug mode is ON! Break at only 5 example')
            break
    torch.cuda.empty_cache()
    return results_dict
...
def inference(
    ...
    comp_soft_label = False if labeling_prob is None else True
    ...
    if load_prediction_from_cache:
        predictions = torch.load(os.path.join(output_folder, cache_name), map_location=torch.device("cpu"))['predictions']
    else:
        predictions = compute_on_dataset(model, data_loader, device, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER, timer=inference_timer, debug=cfg.DEBUG)
    ...
    extra_args = dict(
        ...
        stl_eval_general=cfg.MODEL.STL_EVAL_GENERAL,
        monitor_mean_recall=cfg.TRAIN.MONITOR_MEAN_RECALL,
